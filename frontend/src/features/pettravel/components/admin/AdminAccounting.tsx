...
interface AdminAccountingProps {
  ...
  isPromotionsPolicyLoading: boolean;
  isPromotionsPolicyVerified: boolean;
  promotionsPolicyError: string;
  fetchAccountingOverview: () => Promise<void>;
  fetchAccountingJournalEntries: () => Promise<void>;
  fetchPromotions: () => Promise<PromotionsPolicy | null>;
...
export function AdminAccounting({
  ...
  promotionsPolicy,
  setPromotionsPolicy,
  isPromotionsPolicyLoading,
  isPromotionsPolicyVerified,
  promotionsPolicyError,
      ...
      {(activeTab === "settings" || activeTab === "admin_promotions") && (() => {
        return (
          <div className="admin-dark-dock w-full p-4 sm:p-6 lg:p-7 flex flex-col gap-6 animate-fade-in text-xs">
            ...
            {!isPromotionsPolicyVerified && (
              <div className="rounded-2xl border border-amber-500/40 bg-amber-500/10 p-4 text-amber-200" role="alert">
                <div className="font-bold">
                  {isPromotionsPolicyLoading ? "Đang tải chính sách giá…" : "Chưa thể xác thực chính sách giá"}
                </div>
                {!isPromotionsPolicyLoading && (
                  <div className="mt-1 flex flex-wrap items-center gap-3">
                    <span>{promotionsPolicyError || "Dữ liệu chính sách chưa sẵn sàng."}</span>
                    <button type="button" className="underline font-bold" onClick={() => void fetchPromotions()}>
                      Thử tải lại
                    </button>
                  </div>
                )}
            ...
            {/* Authoritative policy summary */}
            {isPromotionsPolicyVerified && <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {showPromotionsForm && isPromotionsPolicyVerified && (
              <form onSubmit={handleSavePromotions} className="bg-[#15192e] p-5 rounded-2xl border border-[#293256] flex flex-col gap-4 animate-fade-in">
                ...
                <div className="flex justify-end gap-3 mt-2">
                  ...
                  <button
                    type="submit"
                    className="admin-pill-btn-primary text-xs py-2 px-6 disabled:cursor-wait disabled:opacity-60"
                    disabled={isSavingPromotions}
                    aria-busy={isSavingPromotions}
                  >
                    {isSavingPromotions ? (
                      <span className="inline-flex items-center gap-2"><RefreshCw size={14} className="animate-spin" /> Đang lưu…</span>
                    ) : "Lưu cấu hình"}
                  </button>
                </div>
              </form>
